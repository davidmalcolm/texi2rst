...
.. option:: format (archetype, string-index, first-to-check), -Wformat, -ffreestanding, -fno-builtin
  ...
  by the systems C runtime library,
...
ifunc ("``resolver``")
  ...
  function being implemented, the resolvers declaration is be a
...
noclone
  ...
  cloninga mechanism that produces specialized copies of functions
...
function_vector
  ...
  bits of the subroutines entry address. Each vector table has special
...
dllexport
  ...
  GCCs default behavior is to emit all inline functions with the
  ...
  including the symbol in the DLLs export table such as using a
...
dllimport
  ...
  another affectit can cause the vtable and run-time type information
...
interrupt
  ...
  use_debug_exception_return
    ...
    Return using the ``deret`` instruction.  Interrupt handlers that dont
...
target (``options``)
  ...
  fused-maddno-fused-madd
    ``target("fused-madd")`` function attribute, x86
