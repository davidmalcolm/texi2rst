...
hardware models or configurationsfor example, 68010 vs 68020,
...
.. option:: -mhalf-reg-file
  ...
  Dont allocate any register in the range ``r32``...``r63``.
...
.. option:: -max-vect-align=num
  ...
  interfaces are unaffected if they dont use SIMD vector modes
...
.. option:: -mmedium-calls
  ...
  Dont use less than 25 bit addressing range for calls, which is the
...
.. option:: -mlra-priority-none
  ...
  Dont indicate any priority for target registers.
...
.. option:: -mno-sched-prolog
  ...
  merging of those instruction with the instructions in the functions
...
.. option:: -mfpu=name
  ...
  operations are not generated by GCCs auto-vectorization pass unless
...
.. option:: -mtls-dialect=dialect
  ...
  ``dialect``s are supportedgnu and gnu2.  The
...
.. option:: -mrelax
  ...
  the assemblers command line and the :option:`--relax` option to the
...
.. option:: -msp8
  ...
  In general, you dont need to set this option by hand.
  ...
  propers command line, because the compiler then knows if the device
...
.. option:: -mtiny-stack
  ...
  Only change the lower 8bits of the stack pointer.

.. option:: -nodevicelib

  Dont link against AVR-LibCs device specific library ``libdev.a``.
...
``EIND``
Pointers in the implementation are 16bits wide.
The address of a function or label is represented as word address so
that indirect jumps and calls can target any code address in the
range of 64Ki words.

In order to facilitate indirect jump on devices with more than 128Ki
...
``RAMPY``
``RAMPZ``
Some AVR devices support memories larger than the 64KiB range
that can be accessed with 16-bit pointers.  To access memory locations
outside this 64KiB range, the contentent of a ``RAMP``
...
__AVR_``Device``__
  ...
  the devices name. For example, :option:`-mmcu=atmega8` defines the
...
__AVR_DEVICE_NAME__
  ...
  the devices name. For example, with :option:`-mmcu=atmega8` the macro
...
__AVR_HAVE_JMP_CALL__
  ...
  This is the case for devices with at least 16KiB of program
  memory.

__AVR_HAVE_EIJMP_EICALL____AVR_3_BYTE_PC__
  The device has the ``EIJMP`` and ``EICALL`` instructions.
  This is the case for devices with more than 128KiB of program memory.
  This also means that the program counter
  (PC) is 3bytes wide.

__AVR_2_BYTE_PC__
  The program counter (PC) is 2bytes wide. This is the case for devices
  with up to 128KiB of program memory.
...
.. option:: -momit-leaf-frame-pointer
  ...
  Dont keep the frame pointer in a register for leaf functions.  This
...
.. option:: -mno-specld-anomaly
  ...
  Dont generate extra code to prevent speculative loads from occurring.
...
.. option:: -mno-csync-anomaly
  ...
  Dont generate extra code to prevent CSYNC or SSYNC instructions from
...
.. option:: -mno-id-shared-library
  ...
  Generate code that doesnt assume ID-based shared libraries are being used.
  This is the default.

.. option:: -mleaf-id-shared-library

  Generate code that supports shared libraries via the library ID method,
  but assumes that this library or executable wont link against any other
  ID shared libraries.  That allows the compiler to use faster code for jumps
  and calls.

.. option:: -mno-leaf-id-shared-library

  Do not assume that the code being compiled wont link against any ID shared
...
.. option:: -mfast-fp
  ...
  the IEEE floating-point standards rules for checking inputs against
...
.. option:: -mno-gotplt, -mgotplt
  ...
  With :option:`-fpic` and :option:`-fPIC`, dont generate (do generate)
...
target.  Apples GCC on Darwin does create fat files if multiple
...
.. option:: -Fdir, -F
  ...
  directory that is in a frameworks Frameworks directory.
...
-mmacosx-version-min=``version``
  ...
  If the compiler was built to use the systems headers by default,
...
.. option:: -force_cpusubtype_ALL
  ...
  This causes GCCs output file to have the ALL subtype, instead of
...
.. option:: -msmall-data, -mlarge-data
  ...
  heap instead of in the programs data segment.
...
.. option:: -minline-plt
  ...
  Enable inlining of PLT entries in function calls to functions that are
  not known to bind locally.  It has no effect without :option:`-mfdpic`.
  Its enabled by default if optimizing for speed and compiling for
...
.. option:: -mgprel-ro
  ...
  that is known to be in read-only sections.  Its enabled by default,
  ...
  for a GOT entry for the referenced symbol, so its more likely to be a
  win.  If it is not, :option:`-mno-gprel-ro` can be used to disable it.

.. option:: -multilib-library-pic

  Link with the (library, not FD) pic libraries.  Its implied by
...
.. option:: -malign-labels
  ...
  is enabled.  It doesnt create new packets; it merely adds NOPs to
  existing ones.
...
.. option:: -msoft-float
  ...
  targets.  Normally the facilities of the machines usual C compiler are
...
.. option:: -mgnu-ld
  ...
  :option:`--with-ld` configure option, GCCs program search path, and
  finally by the users :envvar:`PATH`.  The linker used by GCC can be printed
...
.. option:: -mhp-ld
  ...
  configure option, GCCs program search path, and finally by the users
...
.. option:: -munix=unix-std
  ...
  as appropriate.  Most GNU software doesnt provide this capability.
...
.. option:: -static
  ...
  libdld.sl.  There isnt an archive version of libdld.sl.  Thus,
...
.. option:: -mgnu-as, -mno-gnu-as
  ...
  Generate (or dont) code for the GNU assembler.  This is the default.
...
.. option:: -mgnu-ld, -mno-gnu-ld
  ...
  Generate (or dont) code for the GNU linker.  This is the default.
...
.. option:: -mvolatile-asm-stop, -mno-volatile-asm-stop
  ...
  Generate (or dont) a stop bit immediately before and after volatile asm
  statements.

.. option:: -mregister-names, -mno-register-names

  Generate (or dont) in, loc, and out register names for
...
.. option:: -mfused-madd, -mno-fused-madd
  ...
  Do (dont) generate code that uses the fused multiply/add or multiply/subtract
  instructions.  The default is to use these instructions.

.. option:: -mno-dwarf2-asm, -mdwarf2-asm

  Dont (or do) generate assembler code for the DWARF 2 line number debugging
...
.. option:: -memregs=number
  ...
  registers, so there is a tradeoff between GCCs ability to fit the
  code into available registers, and the performance penalty of using
  memory instead of registers.  Note that all modules in a program must
  be compiled with the same value for this option.  Because of that, you
  must not use this option with GCCs default runtime libraries.
...
.. option:: -G num, -G
  ...
  doesnt the linker gives an error messageincorrect code is not
  generated.
...
.. option:: -march=arch
  ...
  architectures are selected according to Freescales ISA classification
...
.. option:: -mall-opts
  ...
  Enables all the optional instructionsaverage, multiply, divide, bit
...
.. option:: -mbitops
  ...
  Enables the bit operation instructionsbit test (``btstm``), set
...
.. option:: -mcop32
  ...
  Enables the 32-bit coprocessors instructions.

.. option:: -mcop64

  Enables the 64-bit coprocessors instructions.
...
.. option:: -mtiny=n
  ...
  register.  The default for this option is 4, but note that theres a
...
.. option:: -mxgot, -mno-xgot
  ...
  file accesses more than 64ks worth of GOT entries.  Very few do.
...
.. option:: -mr10k-cache-barrier=setting
  ...
  controls GCCs implementation of this workaround.  It assumes that
  aborted accesses to any byte in the following regions does not have
  side effects:

  * the memory occupied by the current functions stack frame;

    * the memory occupied by an incoming stack argument;

    * the memory occupied by an object with a link-time-constant address.

  It is the kernels responsibility to ensure that speculative
...
.. option:: -mmcount-ra-address, -mno-mcount-ra-address
  ...
  calling functions return address.  When enabled, this option extends
...
.. option:: -mmcu=
  ...
  command line.  The scripts name is the name of the MCU with
...
-msymbol-as-address
  ...
  programs, which run faster than when the option isnt used.  However, the
...
.. option:: -mcpu=cpu_type
  ...
  code for that CPU; it doesnt necessarily reflect the actual hardwares
  ...
  enable or disable them individually if youre sure itll work in your
  environment.
...
.. option:: -minsert-sched-nops=scheme
  ...
  pad
    ...
    according to the schedulers grouping.
...
.. option:: -mveclibabi=type
  ...
  which specifies to use IBMs Mathematical Acceleration Subsystem
...
.. option:: -msmall-data-limit=N
  ...
  not overflow.  Also when the small data area is used one of the RXs
...
.. option:: -mas100-syntax, -mno-as100-syntax
  ...
  Renesass AS100 assembler.  This syntax can also be handled by the GAS
...
.. option:: -mallow-string-insns, -mno-allow-string-insns
  ...
  an I/O register.  (See section 12.2.7 of the RX62N Group Users Manual
...
.. option:: -mbackchain, -mno-backchain
  ...
  Store (do not store) the address of the callers frame as backchain pointer
  into the callees stack frame.
...
.. option:: -mwarn-framesize=framesize
  ...
  this is a compile-time check it doesnt need to be a real problem when the program
...
.. option:: -m4al
  ...
  :option:`-dsp` to the assembler.  GCC doesnt generate any DSP
...
.. option:: -musermode, -mno-usermode
  ...
  Dont allow (allow) the compiler generating privileged mode code.  Specifying
...
.. option:: -mpt-fixed
  ...
  Assume ``pt*`` instructions wont trap.  This generally generates
...
.. option:: -mcbranch-force-delay-slot
  ...
  slot with a ``nop`` if a suitable instruction cant be found.  By default
...
.. option:: -mno-faster-structs, -mfaster-structs
  ...
  ABI.  Thus, its intended only for use on targets where the developer
...
.. option:: -mmemory-model=mem-model
  ...
  architecture manual, as set in the processors ``PSTATE.MM`` field.
...
.. option:: -mlong-calls, -mno-long-calls
  ...
  far away, the compiler always loads the functions address into a
...
.. option:: -msoft-float
  ...
  Normally the facilities of the machines usual C compiler are used, but
  this cant be done directly in cross-compilation.  You must make your
...
.. option:: -mvect8-ret-in-mem
  ...
  default on Solaris8 and 9 and VxWorks to match the ABI of the Sun
  Studio compilers until version 12.  Later compiler versions (starting
  with Studio 12 Update1) follow the ABI used by other x86 targets, which
  is the default on Solaris10 and later.  Only use this option if
...
.. option:: -mfentry
  ...
  isnt possible at the moment for :option:`-mfentry` and :option:`-pg`.
...
.. option:: -mskip-rax-setup
  ...
  misbehave or jump to a random location.  GCC 4.4 or newer dont have
...
.. option:: -mdll
  ...
  specifies that a DLLa dynamic link libraryis to be
...
.. option:: -fno-set-stack-executable
  ...
  the executable flag for the stack used by nested functions isnt
  set. This is necessary for binaries running in kernel mode of
  Microsoft Windows, as there the User32 API, which is used to set executable
  privileges, isnt available.
...
.. option:: -mfused-madd, -mno-fused-madd
  ...
  sensitive to the compilers ability to combine multiply and add/subtract
  operations.
...
.. option:: -mlongcalls, -mno-longcalls
  ...
  instructionslook at the disassembled object code to see the actual
