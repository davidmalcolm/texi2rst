...
of these are not GCC bugs per seif they were, we would fix them.
But the result for a user may be like the result of a bug.

Some of these problems are due to bugs in other software, some are
missing features that are too much work to add, and some are places
where peoples opinions differ as to what is best.
...
.. toctree::
  ...
  GCC uses corrected versions of system header files.
                          This is necessary, but doesnt always work smoothly. <fixed-headers>
  GCC uses the system C library, which might not be
                          compliant with the ISO C standard. <standard-libraries>
  Regrettable things we cant change, but not quite bugs. <disappointments>
